    ...
    <my-tab-bar>
      ...
      <template v-slot:center>
        <view class="positoin-content" @click="gotoPositionList">
          ...
          <view :class="{colorBlack:switchColor,'positoin-text':true}" :style="'max-width:' + tabBarCenterMaxWidth + 'px;'"> {{address}} </view>
        </view>
      </template>
    </my-tab-bar>
    <!-- 轮播图 -->
      ...
      if (!this.address) {
        uni.getLocation({
          ...
          success:(res) => {
            ...
            let qqmapsdk = new QQMapWX({key : "3RDBZ-WSOL2-E36UY-CRAAF-SM2X3-DDFWE"})
